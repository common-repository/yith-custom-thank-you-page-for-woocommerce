...
<h2 class="order_details_title">
	<?php echo apply_filters( 'ctpw_order_details_title', esc_html__( 'Order Details', 'yith-custom-thank-you-page-for-woocommerce' ) ); ?>
</h2>
<table class="shop_table order_details">
	<thead>
		<tr>
			<th class="product-name">
				<?php esc_html_e( 'Product', 'yith-custom-thank-you-page-for-woocommerce' ); ?>
			</th>
					...
					<td class="product-name">
						...
						if ( $_product && $_product->exists() && $_product->is_downloadable() && $order->is_download_permitted()) {
							...
							foreach ( $download_files as $download_id => $file ) {
								...
								$links[] = '<small><a href="' . esc_url( $file['download_url'] ) . '">' . sprintf(
									esc_html__( 'Download file %s', 'yith-custom-thankyou-page-for-woocommerce'),
								( count( $download_files ) > 1 ? ' ' . $i . ': ' : ': ' ) ) . esc_html( $file['name'] ) . '</a></small>';
							}
